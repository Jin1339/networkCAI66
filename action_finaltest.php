...
<body>
    <?php
        ...
        if($data != 0){
            ...
            $sqlexamm = "INSERT INTO `tbl_answermyuser`(`id_user`, `subjectId` , `Article1`, `Article2`, `Article3`, `Article4`, `Article5`, `Article6`, `Article7`, `Article8`, `Article9`, `Article10`, `Article11`, `Article12`, `Article13`, `Article14`, `Article15`, `datetime`) VALUE($id_user, '2' , $exam_1, $exam_2, $exam_3, $exam_4, $exam_5, $exam_6, $exam_7, $exam_8, $exam_9, $exam_10, $exam_11, $exam_12, $exam_13, $exam_14, $exam_15, '$dateTime')";
        ...
        // 1. รวมข้อที่ได้ 1 ทั้งหมด  ถูก
        // 2. รวมข้อที่ได้ 0 ทั้งหมด ผิด
        // 3. รวมคะแนนได้ทั้งหมดเท่าไร รวม
        ...
        $sql2 = "SELECT * FROM tbl_rptsubject WHERE tbl_rptsubject.userId = $id_user AND subjectId = '2'";
        $q2 = mysqli_query($con, $sql2);
        $data2 = mysqli_num_rows($q2);
        if($data2 != 0){
            $sqlsroce_up = "UPDATE `tbl_rptsubject` SET `number_correct`='$scoreall_1' ,`number_wrong`='$scoreall_0',`pass_criteria`='9' ,`score`='$scoresum' ,`adm_results`='$adm_results' WHERE `userId`='$id_user' AND subjectId = 2";
            $qsroce_up = mysqli_query($con , $sqlsroce_up);
        }else{
            $sqlsroce = "INSERT INTO tbl_rptsubject(userId, subjectId, number, number_correct, number_wrong, pass_criteria, score, adm_results) VALUES ('$id_user' , '2' , '15' , '$scoreall_1' , '$scoreall_0' , '12' , '$scoresum' , '$adm_results')";
        ...
        $sqlpercent = "SELECT * FROM tbl_rptpercent WHERE tbl_rptpercent.userId = $id_user AND subjectId = '2'";
        $qpercent = mysqli_query($con, $sqlpercent);
        $datapercent = mysqli_num_rows($qpercent);
        $percent_criteria = 100/15*12;
        $percent_score = 100/15*$scoresum;

        if($datapercent != 0){
            $sqlpercent_up = "UPDATE `tbl_rptpercent` SET `score`='$scoresum',`percent_score`='$percent_score',`adm_results`='$adm_results' WHERE `userId`='$id_user' AND subjectId = '2'";
            $qpercent_up = mysqli_query($con , $sqlpercent_up);
        }else{
            $sqlpercent = "INSERT INTO `tbl_rptpercent`(`userId`, `subjectId`, `all_exams`, `percent_allexams`, `pass_criteria`, `percent_criteria`, `score`, `percent_score`, `adm_results`) VALUES ('$id_user','2','15','100','12','80','$scoresum','$percent_score','$adm_results')";
            $qpercent = mysqli_query($con , $sqlpercent);
        }
        ...
        $sqlacademic_up = "UPDATE `tbl_academicresults` SET `score_subject2`='$scoresum',`percent_subject2`='$percent_score' WHERE `userId`='$id_user'";
        $qacademic_up = mysqli_query($con , $sqlacademic_up);

        $sqldo = "SELECT * FROM tbl_academicresults WHERE tbl_academicresults.userId = $id_user";
        ...
        $subject4 = $datada['score_subject4'];
        $subject5 = $datada['score_subject5'];

        $sum_sub = $subject1 + $subject2 + $subject3 + $subject4 + $subject5;
        $percent_sub = 100 / 40 * $sum_sub;
        ...
        $sqlsub_up = "UPDATE `tbl_academicresults` SET `total_score`='$sum_sub',`percent_totalscore`='$percent_sub',`standard`='$adm_resultssub' WHERE `userId`='$id_user'";
        $qsub_up = mysqli_query($con , $sqlsub_up);
